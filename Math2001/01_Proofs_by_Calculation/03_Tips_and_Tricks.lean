...
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1
      = z ^ 2 ^ 2 - z ^ 2 - (z ^ 3 - 2 * z) + 1 := by ring
    _ = (z ^ 2 - 2 + 2) ^ 2 - (z ^ 2 - 2 + 2) - z * (z ^ 2 - 2) + 1 := by ring
    _ = (0 + 2) ^ 2 - (0 + 2) - z * 0 + 1 := by rw [h1]
    _ = 3 := by ring
...
example {t : ℚ} (ht : t ^ 2 - 4 = 0) :
  ...
  calc
    ...
    _ = t ^ 4 - 3 * t ^ 2 + 3 * t ^ 3 - 2 * t - 2 := by ring
    _ = t ^ 2 * (t ^ 2 - 3) + 3 * t ^ 3 - 2 * t - 2 := by ring
    _ = t ^ 2 * (t ^ 2 - 3) + t * (3 * t ^ 2 - 2 ) - 2 := by ring
    _ = (t ^ 2 - 4 + 4) * (t ^ 2 - 4 + 1) + t * (3 * (t ^ 2 - 4 + 4) - 2 ) - 2 := by ring
    _ = (0 + 4) * (0 + 1) + t * (3 * (0 + 4) - 2) - 2 := by rw [ht]
    _ = 4 + t * (3 * (0 + 4) - 2) - 2 := by ring
    _ = 10 * t + 2 := by ring
...
example {x y : ℝ} (h1 : x + 3 = 5) (h2 : 2 * x - y * x = 0) : y = 2 :=
  calc
    ...
    _ = 2 - (2 - y) := by ring
    _ = 2 - (2 - y) * 2 / 2 := by ring
    _ = 2 - (2 - y) * (5 - 3) / 2 := by ring
    _ = 2 - (2 - y) * (x + 3 - 3) / 2 := by rw [←h1]
    _ = 2 - (2 - y) * x / 2 := by ring
...
example {p q r : ℚ} (h1 : p + q + r = 0) (h2 : p * q + p * r + q * r = 2) :
  ...
  calc
    p ^ 2 + q ^ 2 + r ^ 2
      = p ^ 2 + q ^ 2 + r ^ 2 + 2 * (p * q + p * r + q * r) - 2 * (p * q + p * r + q * r) := by ring
    _ = p ^ 2 + q ^ 2 + r ^ 2 + 2 * (p * q + p * r + q * r) - 2 * 2 := by rw [h2]
    _ = p ^ 2 + q ^ 2 + r ^ 2 + 2 * (p * q + p * r) + 2 * q * r - 4 := by ring
    _ = p ^ 2 + q ^ 2 + r ^ 2 + 2 * p * q + 2 * (p * r + q * r) - 4 := by ring
    _ = p ^ 2 + q ^ 2 + r ^ 2 + 2 * p * q + 2 * (p + q) * r - 4 := by ring
    _ = p ^ 2 + 2 * p * q + q ^ 2 + r ^ 2 + 2 * (p + q) * r - 4 := by ring
    _ = (p + q) ^ 2 + r ^ 2 + 2 * (p + q) * r - 4 := by ring
    _ = (p + q) ^ 2 + 2 * (p + q) * r + r ^ 2 - 4 := by ring
    _ = (p + q) ^ 2 + 2 * (p + q) * r + r ^ 2 - 4 := by ring
    _ = (p + q + r) ^ 2 - 4 := by ring
    _ = 0 ^ 2 - 4 := by rw [h1]
    _ = -4 := by ring
